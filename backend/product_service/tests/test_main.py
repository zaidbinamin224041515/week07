# week03/example-3/backend/product_service/tests/test_main.py
...
def setup_database_for_tests():
    """
    Ensures the Product Service's database is reachable and its tables are created
    ...
    for i in range(max_retries):
        try:
            logging.info(f"Product Service Tests: Attempting to connect to PostgreSQL for test setup (attempt {i+1}/{max_retries})...")
            # Explicitly drop all tables first to ensure a clean slate for the session
            Base.metadata.drop_all(bind=engine)
            logging.info("Product Service Tests: Successfully dropped all tables in PostgreSQL for test setup.")

            # Then create all tables required by the application
            Base.metadata.create_all(bind=engine)
            logging.info("Product Service Tests: Successfully created all tables in PostgreSQL for test setup.")
            break
        except OperationalError as e:
            logging.warning(f"Product Service Tests: Test setup DB connection failed: {e}. Retrying in {retry_delay_seconds} seconds...")
            time.sleep(retry_delay_seconds)
            if i == max_retries - 1:
                pytest.fail(f"Could not connect to PostgreSQL for Product Service test setup after {max_retries} attempts: {e}")
        except Exception as e:
            pytest.fail(f"Product Service Tests: An unexpected error occurred during test DB setup: {e}", pytrace=True)
    
    yield # Yield control to the tests

    # Optional: If you want to explicitly drop tables after all tests in the session, uncomment:
    # logging.info("Product Service Tests: Dropping tables from main PostgreSQL after test session.")
    # Base.metadata.drop_all(bind=engine)
...
def client():
    ...
    """
    # Temporarily set Azure environment variables for testing.
    # In real tests, you might mock BlobServiceClient directly.
    # For now, ensuring these are set to non-None values to allow BlobServiceClient init.
...
def mock_azure_blob_storage():
    ...
    with patch('app.main.BlobServiceClient') as mock_blob_service_client:
        ...
        mock_blob_client.url = "https://testaccount.blob.core.windows.net/test-images/mock_blob.jpg"
...
def test_create_product_success(client: TestClient, db_session_for_test: Session):
    ...
    assert float(response_data["price"]) == test_data["price"] # Convert to float for comparison
    ...
    db_product = db_session_for_test.query(Product).filter(Product.product_id == response_data["product_id"]).first()
...
def test_get_product_success(client: TestClient, db_session_for_test: Session):
    ...
    product_data = {"name": "Get Product Test", "description": "Temp desc", "price": 15.00, "stock_quantity": 20, "image_url": "http://example.com/get_test.jpg"}
...
def test_update_product_partial(client: TestClient, db_session_for_test: Session):
    ...
    product_data = {"name": "Original Name", "description": "Original Desc", "price": 10.00, "stock_quantity": 10, "image_url": "http://example.com/original.gif"}
    create_response = client.post("/products/", json=product_data)
    product_id = create_response.json()["product_id"]
    ...
    assert updated_product["image_url"] == "http://example.com/new_image.bmp" # No SAS token here as it's a PUT input
